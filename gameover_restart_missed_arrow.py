...
def draw_arrows():
    ...
    for (x, y, z, angle_deg, pitch_deg, vz) in arrows:
        ...
        if abs(x) < 4000 and abs(y) < 4000 and z > 0:
            ...
            # Arrow went out of bounds or hit ground - count as missed
            missed_arrows += 1
            # Check game over condition for missed arrows
            if missed_arrows >= MAX_MISSED_ARROWS:
                game_over = True

    arrows = updated
    
    # Draw all remaining arrows
    for (x, y, z, angle_deg, pitch_deg, vz) in arrows:
...
def update_enemies():
    ...
    for (x, y, z) in enemies:
        ...
        # If reached the castle gate, respawn and reduce castle health
        if y <= 10:
            castle_health -= 1
            spawn_enemy()
            # Check game over condition when castle health reaches 0
            if castle_health <= 0:
                game_over = True
        else:
...
def check_arrow_enemy_collision():
    global arrows, enemies, kill_count, missed_arrows, game_over
    updated_arrows = []
...
def keyboardListener(key, x, y):
    global player_angle, player_pitch, god_mode, god_players, game_over, missed_arrows, castle_health, kill_count, arrows
    if key == b'a':
        player_angle += 5
    elif key == b'd':
        player_angle -= 5
...
def showScreen():
    ...
    draw_text(10, 650, f"Castle Health: {castle_health}")
    draw_text(10, 590, f"Missed Arrows: {missed_arrows}/{MAX_MISSED_ARROWS}")
    
    if game_over:
        draw_text(300, 400, "GAME OVER!", font=GLUT_BITMAP_HELVETICA_24)
        draw_text(300, 360, "Press 'R' to Restart", font=GLUT_BITMAP_HELVETICA_18)
        if castle_health <= 0:
            draw_text(300, 320, "Castle destroyed! Too many enemies reached the fire!", font=GLUT_BITMAP_HELVETICA_12)
        elif missed_arrows >= MAX_MISSED_ARROWS:
            draw_text(300, 320, "Too many missed arrows!", font=GLUT_BITMAP_HELVETICA_12)
